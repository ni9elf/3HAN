...
print ('loading files...')
train_articles = cPickle.load(open('Dataset/train_articles_seq.p', 'rb'))
train_headlines = cPickle.load(open('Dataset/train_headlines_seq.p', 'rb'))
train_y = cPickle.load(open('Dataset/train_y.p', 'rb'))

val_articles = cPickle.load(open('Dataset/val_articles_seq.p', 'rb'))
...
#-----------------------------------------------------------------------------------------------------------------------------        
print ('Build model...')
model, model1 = fhan3_pretrain(MAX_NB_WORDS, WORD_LIMIT, SENT_LIMIT, EMBEDDING_DIM, WORDGRU, embedding_matrix, DROPOUTPER)
print model.summary()
...
class ModelSave(callbacks.Callback):
    def on_epoch_end(self, epoch, logs={}):
        model1.save('SavedModels/han3_pretrain_wordEncoder_epoch_{}.h5'.format(epoch))
        model.save('SavedModels/han3_pretrain_epoch_{}.h5'.format(epoch))
...
model.fit([train_articles , train_headlines], train_y, validation_data=([val_articles, val_headlines], val_y), shuffle=True, batch_size=32, epochs=1000, callbacks=callbacks)
...
test_articles = cPickle.load(open('Dataset/test_articles_seq.p', 'rb'))
...
print model.evaluate([test_articles ,test_headlines], test_y, batch_size=32)
